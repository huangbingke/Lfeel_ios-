...
- (instancetype)init {
    self = [super init];
    if (self) {
        self = [[[NSBundle mainBundle] loadNibNamed:@"TZDatePickerView" owner:self options:nil] firstObject];
        // 初始化设置
...
- (IBAction)gotoSrceenOrderClick:(id)sender {
    ...
    if (self.complateBlock) {
        self.complateBlock(self.beginDateLable.text,self.endDateLable.text);
    }
}
...
- (void)dealloc {
    ...
    SLLog(@"TZDatePickerView销毁了");
}
@end


@interface TZDatePickerView_Singel ()
@property (nonatomic, strong) UIView *bgView;
/** 提示lable */
@property (weak, nonatomic) IBOutlet UILabel *tipLable;
/** datePicker */
...
@implementation TZDatePickerView_Singel
...
- (instancetype)init {
    ...
    if (self) {
        ...
        // 初始化设置
        UIWindow *window = [UIApplication sharedApplication].keyWindow;
        self.frame = CGRectMake(0, mScreenHeight, mScreenWidth, 265);
        [window addSubview:self.bgView];
        [window addSubview:self];
    }
    return self;
...
- (IBAction)rightClick:(id)sender {
    [self hide];
    if (self.complateBlock) {
        self.complateBlock([self.formatter stringFromDate:self.datePicker.date]);
    }
}
